...
def trash(mail, main):
    print_and_speak("Welcome to the Trash. You can say 'read' followed by the number to read an email, 'delete' followed by the number to delete an email, back to navigate to main menu or 'exit' to quit.")
    while True:
        ...
        if command:
            if "read" in command:
                ...
                if len(parts) == 2:
                    ...
                    if num is not None:
                        content = fetch_email_from_trash(mail, num)
                ...
                if len(parts) == 2:
                    num = convert_words_to_number(parts[1])
                    if num is not None:
                        result = delete_email_from_trash(mail, num)
...
def fetch_email_from_trash(mail, index):
...
def delete_email_from_trash(mail, index):
    mail.select('[Gmail]/Trash')
    status, data = mail.search(None, 'ALL')
    email_ids = data[0].split()
    num_emails = len(email_ids)
