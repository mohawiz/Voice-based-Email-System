...
def read_emails_from_folder_2(mail, folder_name):
    while True:
        emails = fetch_emails_from_folder(mail, folder_name)
        for index, subject in emails:
            print_and_speak(f"{index}: {subject}")
        
        print_and_speak(f"Welcome to the {folder_name}. You can say 'read' followed by the number to read an email, 'delete' followed by the number to delete an email, 'menu' to return to the main menu, or 'logout' to log out.")
        command = recognize_speech()
        if command:
            if "read" in command:
                parts = command.split()
                ...
                if len(parts) == 2:
                    num = convert_words_to_number(parts[1])
                    if num is not None:
                        print_and_speak(f"Are you sure you want to delete email number {num}? Say 'yes' to confirm or 'no' to cancel.")
                        confirmation = recognize_speech()
                        if confirmation and 'yes' in confirmation.lower():
                            result = delete_email(mail, num)
                            print_and_speak(result)
                        else:
                            print_and_speak("Email deletion cancelled.")
                    else:
                ...
                print_and_speak("Unknown command. Please say 'read' followed by the number, 'delete' followed by the number, 'menu' to return to the main menu, or 'logout' to log out.")
        else:
            print_and_speak("No command heard. Please try again.")
